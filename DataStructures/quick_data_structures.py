...
def edges(v,n,acyclic=True):
    return [random.sample(list(v),k=2) if acyclic else random.choices(list(v),k=2) for _ in range(n)]
...
def adj_mat(v,e,directed=False,weighted=False,acyclic=True):
    ...
    for triplet in edgs:
        ...
        if mat[verts[triplet[to]]][verts[triplet[frm]]] == 0:
            mat[verts[triplet[to]]][verts[triplet[frm]]] = (-1*(triplet[2]**int(weighted)))**int(directed)
    return mat
